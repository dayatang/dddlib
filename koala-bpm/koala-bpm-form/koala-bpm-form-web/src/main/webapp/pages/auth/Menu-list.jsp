...
<body style="padding:10px;height:100%; text-align:center;">
  ...
  $(function() {
		...
		$.getJSON(rootPath+"/auth/Menu/findMenuType.koala", function(result) {
		...
		var grid = $("#maingrid").ligerGrid({
			...
			url : rootPath+'/auth/Menu/findAllMenuTreeRows.koala',
			sortName : 'id',
			width : '98%',
			height : '100%',
			heightDiff : -10,
		...
		function changeSort() {
			...
			$.ajax({
				type : "post",
				data : data.join("&"),
				url : rootPath+"/auth/Menu/updateMenuOrder.koala"
			});
		}
		...
		function f_delete() {
			...
			if (grid.getCheckedRows().length = 1) {
				Koala.ajax({
					type : "post",
					url : rootPath+"/auth/Menu/del.koala",
					loading : "正在删除中...",
		...
		function saveMenu() {
			...
			var data;
			if (isEdit) {
				url = rootPath+"/auth/Menu/update.koala";
				data = [ {
					name : 'resVO.id',
				...
				if (!selectedRow) {
					url = rootPath+"/auth/Menu/add.koala";
					data = {
						'resVO.name' : $("#name").val(),
					...
					url = rootPath+"/auth/Menu/addAndAssignParent.koala";
					data = {
		...
		$.getJSON(rootPath+"/auth/Menu/getIconNames.koala", function(result) {

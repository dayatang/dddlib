...
public class EmployeeControllerTest {
	...
	public void testPagingQuery() {
		initEmployeePage();
		when(employeeApplication.pagingQueryEmployees(new EmployeeDTO(), 1, 10)).thenReturn(employeePage);
		assertEquals(employeePage.getResult(), employeeController.pagingQuery(1, 10, new EmployeeDTO()).get("Rows"));
	}
	...
	public void testPagingQueryByOrganization() {
		...
		when(baseApplication.getEntity(Organization.class, 1L)).thenReturn(organization);
		when(employeeApplication.pagingQueryEmployeesByOrganization(new EmployeeDTO(), organization, 1, 10)).thenReturn(employeePage);
		assertEquals(employeePage.getResult(), employeeController.pagingQueryByOrganization(1, 10, new EmployeeDTO(), 1L, false).get("Rows"));
	}
	...
	public void testPagingQueryByOrganizationAndChildren() {
		...
		when(baseApplication.getEntity(Organization.class, 1L)).thenReturn(organization);
		when(employeeApplication.pagingQueryEmployeesByOrganizationAndChildren(new EmployeeDTO(), organization, 1, 10)).thenReturn(employeePage);
		assertEquals(employeePage.getResult(), employeeController.pagingQueryByOrganization(1, 10, new EmployeeDTO(), 1L, true).get("Rows"));
	}
	...
	public void testCreateEmployee() {
		...
		verify(employeeApplication, only()).createEmployeeWithPost(employee, post);
	}
	...
	public void testCatchSnIsExistExceptionWhenCreateEmployee() {
		...
		when(baseApplication.getEntity(Post.class, postId)).thenReturn(post);
		doThrow(new SnIsExistException()).when(employeeApplication).createEmployeeWithPost(employee, post);
		assertEquals("员工编号: " + employee.getSn() + " 已被使用！", employeeController.createEmployee(employee, postId).get("result"));
	}
	...
	public void testIdNumberIsExistExceptionWhenCreateEmployee() {
		...
		when(baseApplication.getEntity(Post.class, postId)).thenReturn(post);
		doThrow(new IdNumberIsExistException()).when(employeeApplication).createEmployeeWithPost(employee, post);
		assertEquals("不能使用与其他人一样的证件号码！", employeeController.createEmployee(employee, postId).get("result"));
	}
	...
	public void testExceptionWhenCreateEmployee() {
		...
		when(baseApplication.getEntity(Post.class, postId)).thenReturn(post);
		doThrow(new RuntimeException()).when(employeeApplication).createEmployeeWithPost(employee, post);
		assertEquals("保存失败！", employeeController.createEmployee(employee, postId).get("result"));
	}
	...
	public void testSnIsExistExceptionWhenUpdateEmployee() {
		...
		doThrow(new SnIsExistException()).when(baseApplication).updateParty(employee);
		assertEquals("员工编号: " + employee.getSn() + " 已被使用！", employeeController.updateEmployee(employee).get("result"));
	}
	...
	public void testIdNumberIsExistExceptionWhenUpdateEmployee() {
		...
		doThrow(new IdNumberIsExistException()).when(baseApplication).updateParty(employee);
		assertEquals("不能使用与其他人一样的证件号码！", employeeController.updateEmployee(employee).get("result"));
	}
	...
	public void testExceptionWhenUpdateEmployee() {
		...
		doThrow(new RuntimeException()).when(baseApplication).updateParty(employee);
		assertEquals("修改失败！", employeeController.updateEmployee(employee).get("result"));
	}
	...
	public void testTransformPost() {
		...
		verify(employeeApplication).transformPost(employee, new HashSet<ResponsiblePostDTO>(Arrays.asList(responsiblePostDTOs)));
	}
	...
	public void testHasPrincipalPostYetExceptionWhenTransformPost() {
		...
		doThrow(new HasPrincipalPostYetException()).when(employeeApplication)
			.transformPost(employee, new HashSet<ResponsiblePostDTO>(Arrays.asList(responsiblePostDTOs)));
		assertEquals("该员工已经有主任职岗位！", employeeController.transformPost(employeeId, responsiblePostDTOs).get("result"));
	}
	...
	public void testEmployeeMustHaveAtLeastOnePostExceptionWhenTransformPost() {
		...
		doThrow(new EmployeeMustHaveAtLeastOnePostException()).when(employeeApplication)
			.transformPost(employee, new HashSet<ResponsiblePostDTO>(Arrays.asList(responsiblePostDTOs)));
		assertEquals("必须保证每名员工至少在一个岗位上任职！", employeeController.transformPost(employeeId, responsiblePostDTOs).get("result"));
	}
	...
	public void testExceptionWhenTransformPost() {
		...
		doThrow(new RuntimeException()).when(employeeApplication)
			.transformPost(employee, new HashSet<ResponsiblePostDTO>(Arrays.asList(responsiblePostDTOs)));
		assertEquals("调整职务失败！", employeeController.transformPost(employeeId, responsiblePostDTOs).get("result"));
	}
	...
	public void testGet() {
		...
		PowerMockito.when(EmployeeDTO.generateDtoBy(employee)).thenReturn(employeeDTO);
		
		assertEquals(employeeDTO, employeeController.get(employeeId).get("data"));
	}
	...
	public void testGetPostsByEmployee() {
		...
		assertEquals(dtos, employeeController.getPostsByEmployee(employeeId).get("data"));
	}
	...
	public void testTerminateEmployee() {
		...
		verify(baseApplication, only()).terminateParty(dto.transFormToEmployee());
	}

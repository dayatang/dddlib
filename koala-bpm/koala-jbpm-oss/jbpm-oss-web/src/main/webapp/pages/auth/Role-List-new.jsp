...
<body style="padding:10px;height:100%; text-align:center;">
  ...
  <div id="treeDiv">
	...
	<div id="assignRolesToUserDialog" style="display: none;">
	  <div id="dialogSearch">
	  	角色名称：<input id="dialogRoleNameForSearch" type="text" class="form-control input-sm" style="width:150px; display:inline; margin-right:15px;"/>
  ...
  $(function() {
		...
		var grid = $("#roleGrid").ligerGrid({
			...
			url : rootPath+'/auth/Role/pageJson.koala',
			sortName : 'id',
			rownumbers : true,
			width : '98%',
			height : '100%',
		...
		function toolbarBtnItemClick(item) {
			...
			case "user":
				if (selectRow()) {
					$("#userGrid").openTab(rootPath+'/auth/User/listByUserId.koala?roleId=' + selectedRow.id, selectedRow.name + '的用户管理', selectedRow.id ,selectedRow.id);
				}
				break;
			case "menuResource":
				if (selectRow()) {
		...
		function loadRoles() {
			roleGrid = $("#assignRolesToUser").ligerGrid({
				...
				pageSize : 10,
				url : rootPath+'/auth/Role/queryNotAssignRoleByUser.koala?userId=' + userId,
				sortName : 'id',
				rownumbers : true,
				width : 450,
				height : 430,
				...
				var menuDialog = $.ligerDialog.open({
					...
					buttons : [{
						...
						onclick : function() {
							...
							$.ajax({
								type : "post",
								url : rootPath+"/auth/Role/assignMenuResources.koala",
								data : postData.join("&"),
		...
		function initResource() {
			top.f_addTab(null, '角色资源', rootPath+'/auth/Role/resourceList.koala?roleVO.id=' + selectedRow.id);
		}
		...
		function removeRole() {
			...
			if (userId > 0) {
				...
				removeUrl = rootPath+'/auth/Role/removeRoleForUser.koala';
				...
				data = data.substr(0, data.length - 1) + '}';
				removeUrl = rootPath+'/auth/Role/del.koala';
				removeData = $.parseJSON(data);
			}
		...
		function saveRole() {
			...
			var data;
			if (isEdit) {
				url = rootPath+"/auth/Role/update.koala";
				data = [ {
					name : 'roleVO.id',
				...
				url = rootPath+"/auth/Role/add.koala";
				data = [ {
					name : 'roleVO.name',
					value : roleName.val()
				}, {
		...
		function assignRoles() {
			...
			$.ajax({
				...
				cache : false,
				dataType : 'json',
				url : rootPath+'/auth/User/assignRoles.koala',
				data : $.parseJSON(data),
				success : function(data) {
					if (data.result == "success") {
						grid.loadData();
		...
		function search() {
			grid = $("#roleGrid").ligerGrid({
				...
				pageSize : 20,
				toolbar : toolbarOptions,
				url : rootPath+'/auth/Role/query.koala',
				sortName : 'id',
				rownumbers : true,
				parms : [ {
		...
		function dialogSearch() {
			roleGrid = $("#assignRolesToUser").ligerGrid({
				...
				pageSize : 10,
				rownumbers : true,
				url : rootPath+'/auth/Role/queryRolesForAssign.koala',
				parms : [ {
					name : "userId",
					value : userId
				}, {

...
<body style="padding:10px;height:100%; text-align:center;">
  ...
  $(function() {
		...
		var grid = $("#userGrid").ligerGrid(
				{
					...
					rownumbers : true,
					url : rootPath+'/auth/User/pageJson.koala?roleId=' + roleId
		...
		function toolbarBtnItemClick(item) {
			...
			case "assignRoles":
				if (selectRow()) {
					$("#userGrid").openTab(rootPath+'/auth/Role/listByUserId.koala?userAccount='
							+ selectedRow.userAccount + '&userId='
							+ selectedRow.id, selectedRow.name + '的角色管理', selectedRow.id ,selectedRow.id);
				}
				break;
			}
		}
		...
		function loadUsers() {
			userGrid = $("#assignUsersToRole").ligerGrid({
				...
				pageSize : 15,
				rownumbers : true,
				url : rootPath+'/auth/User/queryNotAssignUserByRole.koala?roleId=' + roleId,
				width : 450,
				height : 430,
				checkbox : true
			});
		...
		function removeUser() {
			...
			if (roleId > 0) {
				...
				removeUrl = rootPath+'/auth/User/removeUserForRole.koala';
				...
				removeUrl = rootPath+'/auth/User/del.koala';
				removeData = $.parseJSON(data);
			}
		...
		function saveUser() {
			...
			var form;
			if (isEdit) {
				url = rootPath+"/auth/User/update.koala";
				...
				url = rootPath+"/auth/User/add.koala";
		...
		function assignUsers() {
			...
			$.ajax({
				...
				cache : false,
				dataType : 'json',
				url : rootPath+'/auth/Role/assignUsers.koala',
				data : $.parseJSON(data),
				success : function(data) {
					if (data.result == "success") {
						grid.loadData();
		...
		$("#searchButton").click(function() {
			grid = $("#userGrid").ligerGrid({
				...
				pageSize : 20,
				toolbar : toolbarOptions,
				url :rootPath+ "/auth/User/query.koala",
				sortName : 'id',
				rownumbers : true,
				width : '98%',
				height : '100%',
		...
		function dialogSearch() {
			userGrid = $("#assignUsersToRole").ligerGrid({
				...
				pageSize : 15,
				rownumbers : true,
				url : rootPath+"/auth/User/queryUsersForAssign.koala",
				sortName : 'id',
				width : 450,
				height : 430,
				checkbox : true,

$(function() {
		...
		case "user":
			if (selectRow()) {
				top.f_addTab(null, selectedRow.name + '的用户管理',
						'auth-User-list.action?roleId=' + selectedRow.id);
			}
			break;
		case "menuResource":
			if (selectRow()) {
	...
	function loadRoles() {
		roleGrid = $("#assignRolesToUser").ligerGrid({
			...
			pageSize : 10,
			url : 'auth-Role-queryNotAssignRoleByUser.action?userId=' + userId,
			sortName : 'id',
			rownumbers : true,
			width : 450,
			height : 430,
	...
	function initMenuTree() {
		...
		$.getJSON('auth-Menu-findMenuTreeSelectItemByRole.action?time=' + new Date().getTime() + '&roleVO.id='+ selectedRow.id, function(menus) {
			var menuDialog = $.ligerDialog.open({
				...
				buttons : [{
					...
					onclick : function() {
						...
						$.ajax({
							type : "post",
							url : "auth-Role-assignMenuResources.action",
							data : postData.join("&"),
	...
	function initResource() {
		top.f_addTab(null, '角色资源', 'auth-Role-resourceList.action?roleVO.id=' + selectedRow.id);
	}
	...
	function removeRole() {
		...
		if (userId > 0) {
			...
			removeUrl = 'auth-Role-removeRoleForUser.action';
			...
			data = data.substr(0, data.length - 1) + '}';
			removeUrl = 'auth-Role-del.action';
			removeData = $.parseJSON(data);
		}
	...
	function assignRoles() {
		...
		$.ajax({
			...
			cache : false,
			dataType : 'json',
			url : 'auth-User-assignRoles.action',
			data : $.parseJSON(data),
			success : function(data) {
				if (data.result == "success") {
					grid.loadData();
	...
	function dialogSearch() {
		roleGrid = $("#assignRolesToUser").ligerGrid({
			...
			pageSize : 10,
			rownumbers : true,
			url : 'auth-Role-queryRolesForAssign.action',
			parms : [ {
				name : "userId",
				value : userId
			}, {

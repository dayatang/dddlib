package org.openkoala.koala.jbpm.util;
...
public final class EnvConfigHelper extends Properties {
	...
	private EnvConfigHelper() {
		InputStream is = getClass().getResourceAsStream("/META-INF/jbpmDesigner-config.properties");
		try {
			load(is);
		} catch (Exception ce) {
			System.out.println("读取配置文件失败！，请确定jbpmDesigner-config.properties文件存在");
		}
	}

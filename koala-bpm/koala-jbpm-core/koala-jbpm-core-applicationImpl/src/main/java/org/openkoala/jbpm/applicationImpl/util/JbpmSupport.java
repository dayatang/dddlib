...
public class JbpmSupport {
	...
	public void initialize() throws Exception {
		...
		kbase.addKnowledgePackages(kbuilder.getKnowledgePackages());//初始化流程库
		ksession.getWorkItemManager().registerWorkItemHandler("Human Task",humanTaskHandler);
		ksession.getWorkItemManager().registerWorkItemHandler("Service Task",new ServiceTaskHandler());
		System.setProperty("jbpm.usergroup.callback","org.jbpm.task.service.DefaultUserGroupCallbackImpl");//引入流程用户初始化接口
	...
	public List<TaskSummary> findTaskSummaryByGroup(String user,List<String> groups){
		return localTaskService.getTasksAssignedAsPotentialOwner(user,groups,"en-UK");
	}
	...
	public List<ProcessInstanceLog> findActiveProcessInstances(String processIdActual){
		return JPAProcessInstanceDbLog.findActiveProcessInstances(processIdActual);
	}
	...
	public List<VariableInstanceLog> findVariableInstances(long processId,String variableId){
		return JPAProcessInstanceDbLog.findVariableInstances(processId,variableId);
	}
	...
	public Map<String,Object> getGlobalVariable(){
		...
		Map<String,Object> globalMap =  variables.get(KoalaJbpmVariable.KOALA_GLOBAL);
		if(globalMap!=null)values.putAll(globalMap);
		return values;
	}
	
	public Map<String,Object> getPackageVariable(String packageName){
		Map<String,Object> values = new HashMap<String,Object>();
		Map<String,Object> packageMap = variables.get(KoalaJbpmVariable.KOALA_PACKAGE+packageName);
		if(packageMap!=null)values.putAll(packageMap);
		return values;
	}
	
	public Map<String,Object> getProcessVariable(String processId){
		Map<String,Object> values = new HashMap<String,Object>();
		Map<String,Object> processMap =  variables.get(KoalaJbpmVariable.KOALA_PROCESS);
		if(processMap!=null)values.putAll(processMap);
		return values;
	}
	...
	public void initActiveProcess() {
		...
		for (KoalaProcessInfo koalaProcessInfo : list) {
			activeProcessMap.put(koalaProcessInfo.getProcessName(),koalaProcessInfo.getVersionNum());
		}
	}
	...
	public void addProcessToCenter(KoalaProcessInfo processInfo,boolean isActive) throws Exception {
		addKoalaProcessInfo(processInfo);
		if(isActive)activeProcessMap.put(processInfo.getProcessName(),processInfo.getVersionNum());
	}
	...
	private void addKoalaProcessInfoVO(KoalaProcessInfoVO info)throws Exception {

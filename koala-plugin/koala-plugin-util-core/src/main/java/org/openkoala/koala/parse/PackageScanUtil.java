...
public class PackageScanUtil {
	...
	private static final String KOALA_PACKAGE = "org.openkoala.koala";
	...
	public static Map<String, String> scan() {
		if (classes == null) {
			...
			Set<Class<?>> sets = getClasses(KOALA_PACKAGE);
			for (Class set : sets) {
				logger.info("扫描到类:"+set.getName());
				classes.put(set.getSimpleName(), set.getName());
			}
		}
		return classes;
	}
					...
					try {
						...
						while (entries.hasMoreElements()) {
							...
							if (name.startsWith(packageDirName)) {
								...
								if (idx != -1) {
									// 获取包名 把"/"替换成"."
									thePackageName = name.substring(0, idx)
											.replace('/', '.');
								}
								// 如果可以迭代下去 并且是一个包
								if ((idx != -1) || recursive) {

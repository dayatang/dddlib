            ...
            dialog.modal({
                ...
                if(childItem.type == 'HTTP'){
                    params.props = 'trace-filter-active-time->'+tracetime.val()
                        +'@@@trace-timeout->'+tracetimeout.val()+'@@@includeUrls->'+includeUrls.val()
                        +'@@@excludeUrls->'+excludeUrls.val();
                }else if(childItem.type == 'METHOD'){
                    params.props = 'trace-filter-active-time->'+tracetime.val()
                        +'@@@trace-timeout->'+tracetimeout.val()+'@@@detect-clazzs->'+detectClazzs.val()
                        +'@@@detect-packages->'+detectPackages.val();
        ...
        $.jqplot('memchart', datas, {
            axes:{
                xaxis:{
                    ...
                    tickOptions:{formatString:'%m-%d %H'}
                    //tickInterval:'2 hour'
                },
                yaxis: {
                    min:0,

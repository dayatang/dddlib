...
public class RemoteDataPolicyClientHandler {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(RemoteDataPolicyClientHandler.class);
	...
	public synchronized void startup(RuntimeContext context) {
		...
		Map<String, String> props = context.getNodeDef().getDataPolicy()
				.getProperties();
		...
		try {
			...
			LOG.warn("Connection to Server fail! cause:[{}->{}]", e.toString(),
					e.getMessage());
			startFail = true;
		}
	...
	private void processServerCommand(Commond command) {
		...
		try {
			if (LOG.isDebugEnabled())
				LOG.debug("接收到服务端指令[" + commandText + "]指令ID["
						+ command.getCommondID() + "]");
			if (CommondConst.FETCH.equals(commandText)) {

				Map<String, List<Trace>> allCacheTrace = RuntimeContext
						.getContext().getDataCache().getAllCacheTrace();
				...
				if (allCacheTrace.size() > 0) {
					List<String> traceKeys = new ArrayList<String>(
							allCacheTrace.keySet());
				...
				ServerStatusVo status = ServerStatusCollector
						.getServerAllStatus();
				replyServerCommond(clientId, command.getCommondID(), status,
						null);
			} else if (CommondConst.GET_GENERAL_STATUS.equals(commandText)) {//
				replyServerCommond(clientId, command.getCommondID(),
						getGeneralMonitorStatus(), null);
			} else if (CommondConst.GET_JDBC_POOL_STATUS.equals(commandText)) {//
				Map<String, JdbcPoolStatusVo> status = JdbcPoolStatusCollector
						.getInstance().currentAllDataSourceStatus();
				Commond commond = new Commond(CommondConst.REPLY);
				commond.addHeader(CommondConst.CMD_ID, command.getCommondID());
				commond.addHeader(CommondConst.CLIENT_ID, clientId);

				Iterator<Entry<String, JdbcPoolStatusVo>> iterator = status
						.entrySet().iterator();
	 ...
	 * @param e
	 */
	private void replyServerCommond(String clientId, String commondId,
			Serializable data, Exception e) {
		Commond commond = new Commond(CommondConst.REPLY);
		commond.addHeader(CommondConst.CMD_ID, commondId);
		commond.addHeader(CommondConst.CLIENT_ID, clientId);
		if (data != null)
			commond.addData(CommondConst.DEFAULT_DATA, data);
		if (e != null)
			commond.addHeader(CommondConst.ERROR, "发生异常["
					+ e.getClass().getName() + "]异常信息：[" + e.getMessage() + "]");
		sendDataToServer(commond);
	}
	...
	private GeneralMonitorStatusVo getGeneralMonitorStatus() {
		...
		// 在线人数
		Map<String, ActiveUser> users = RuntimeContext.getContext()
				.getDataCache().getActiveSessions();
		// 恶意访问IP
		Iterator<ActiveUser> it = users.values().iterator();
		while (it.hasNext()) {
			ActiveUser user = it.next();
			if (user.isAbNormal())
				vo.getAbnormalIps().add(user.getIp());
		}
		vo.setActiveCount(users.size());
		// 服务器状态
		MonitorTask task = RuntimeContext.getContext().getMonitorTask(
				ServerStatusCollectorTask.TASK_KEY);
		if (task != null) {
			vo.setServerStatusDatas(task.getDatas());
		}
		// 第三方服务
		task = RuntimeContext.getContext().getMonitorTask(
				ServiceConnectionCheckTask.TASK_KEY);
		if (task != null) {

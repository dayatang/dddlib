...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
            var g = this, p = this.options;
            if (!item.id || !g.koalaTabid) return;
            switch (item.id)
            {
                case "close":
                    g.removeTabItem(g.koalaTabid);
                    g.koalaTabid = null;
                    break;
                case "closeother":
                    g.removeOther(g.koalaTabid);
                    break;
                case "closeall":
                    g.removeAll();
                    g.koalaTabid = null;
                    break;
                case "reload":
                    g.selectTabItem(g.koalaTabid);
                    g.reload(g.koalaTabid);
                    break;
            }
        },
        _addTabItemContextMenuEven: function (tabitem)
        {
            ...
            tabitem.bind("contextmenu", function (e)
            {
                ...
                g.koalaTabid = tabitem.attr("tabid");
